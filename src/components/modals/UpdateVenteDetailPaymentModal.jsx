...
function UpdateVenteDetailPaymentModal({ isOpen, onResolve, onReject, vente }) {
  ...
  const onCreate = (data) => {
    const { avance } = data;
    const tv = +avance + vente?.montant;
    onResolve({ _id: vente?._id, montant: tv });
  };
...
export default create(UpdateVenteDetailPaymentModal);
