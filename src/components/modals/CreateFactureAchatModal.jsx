...
function CreateFactureAchatModal({ isOpen, onResolve, onReject, fournisseur }) {
  const defaultValues = {
  ...
  const onCreate = (data) => {
    ...
    const avi = +avance;
    const fd = { ...data, total: +total, avance: avi, restant: +total - avi };
    onResolve(fd);
  };
  ...
  return (
    <>
      <Dialog
        header="Enregistrer Facture Fournisseur"
        visible={isOpen}
...
export default create(CreateFactureAchatModal);
