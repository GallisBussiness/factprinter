...
function UpdateFournisseurModal({ isOpen, onResolve, onReject, fournisseur }) {
  const defaultValues = {
    nom: fournisseur?.nom,
  ...
  const onCreate = (data) => {
    onResolve({ _id: fournisseur?._id, ...data });
  };

  return (
    <>
      <Dialog
        header="Mettre à jour un fournisseur"
        visible={isOpen}
...
export default create(UpdateFournisseurModal);
