...
function UpdateClientModal({ isOpen, onResolve, onReject, client }) {
  const defaultValues = {
    nom: client?.nom,
  ...
  const onCreate = (data) => {
    onResolve({ _id: client?._id, ...data });
  };

  return (
    <>
      <Dialog
        header="Mettre à jour un client"
        visible={isOpen}
...
export default create(UpdateClientModal);
