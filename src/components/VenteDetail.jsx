...
function VenteDetail() {
  ...
  const { mutate: create, isLoading: isLoadingC } = useMutation(
    (data) => createVenteDetail(data),
    {
      onSuccess: (_) => {
        toast.current.show({
          severity: "success",
          summary: "Creation Vente Détail",
  ...
  const { mutate: update } = useMutation(
    (data) => updateVenteDetail(data._id, data.data),
    {
      onSuccess: (_) => {
        toast.current.show({
          severity: "success",
          summary: "Mise à jour VenteDetail",
      ...
      onError: (_) => {
        toast.current.show({
          severity: "error",
          summary: "Mis à jour VenteDetail",
  ...
  const { mutate: deleteV } = useMutation((id) => removeVenteDetail(id), {
    onSuccess: (_) => {
      toast.current.show({
        severity: "success",
        summary: "Suppréssion VenteDetail",
    ...
    return (
      <div className="flex items-center justify-center space-x-2">
        ...
        <button
          ...
          disabled={!selectedVenteDetails || !selectedVenteDetails.length}
        >
      ...
      <div className="datatable-doc mt-4 w-4/5 mx-auto">
        <div className="card">
          ...
          <DataTable
            ...
            dataKey="_id"
            rowHover
            selection={selectedVenteDetails}
            onSelectionChange={(e) => setSelectedVenteDetails(e.value)}
            filters={filters}
            filterDisplay="menu"
            loading={isLoading}
            responsiveLayout="scroll"
            globalFilterFields={["date", "montant"]}
            emptyMessage="Aucune VenteDetail trouvée"
            currentPageReportTemplate="Voir {first} de {last} à {totalRecords} VenteDetails"
            size="small"
          >
            <Column
