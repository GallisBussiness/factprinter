...
function Fournisseurs() {
  ...
  const { mutate: update } = useMutation(
    (data) => updateFournisseur(data._id, data.data),
    {
      onSuccess: (_) => {
        toast.current.show({
          severity: "success",
          summary: "Mise à jour Fournisseur",
      ...
      onError: (_) => {
        toast.current.show({
          severity: "error",
          summary: "Mis à jour Fournisseur",
  ...
  const handleViewFournisseur = (row) =>
    navigate(`/dashboard/fournisseurs/${row._id}`);
  const renderHeader = () => {
    return (
  ...
  const actionBodyTemplate = (rowData) => {
    return (
      <div className="flex items-center justify-center space-x-1">
        <button
          type="button"
          onClick={() => handleViewFournisseur(rowData)}
        ...
        >
          <BsEye className="text-white inline" />
        </button>
        <button
          type="button"
          onClick={() => handleUpdateFournisseur(rowData)}
          className="inline-block px-6 py-3 font-bold text-center text-white uppercase align-middle transition-all rounded-lg cursor-pointer bg-gradient-to-tl from-blue-700 to-blue-300 leading-pro text-xs ease-soft-in tracking-tight-soft shadow-soft-md bg-150 bg-x-25 hover:scale-102 active:opacity-85 hover:shadow-soft-xs"
        >
      ...
      <div className="datatable-doc mt-4 w-4/5 mx-auto">
        <div className="card">
          ...
          <DataTable
            ...
            currentPageReportTemplate="Voir {first} de {last} à {totalRecords} Fournisseurs"
            size="small"
          >
            <Column
